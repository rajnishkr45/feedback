...
<body>
            ...
            <div class="table-data">
                <div class="order">
                    <div class="head">
                        ...
                        <form id="search-form">
                            <input type="hidden" id="class-start-time" value="<?php echo date('Y-m-d H:00:00'); ?>">
    ...
    <script>
        $(document).ready(function () {
            ...
            // Helper function to check if the current time is within the 15-minute window
            function isValidTime(classStartTime) {
                const currentTime = new Date(); // Current time
                const validStartHours = [10, 11, 12, 13, 14, 15, 16]; // Class start times: 10 AM to 4 PM in 24-hour format

                // Parse classStartTime
                const classStart = new Date(classStartTime);
                const classHour = classStart.getHours();

                // Check if the class hour is valid
                if (!validStartHours.includes(classHour)) {
                    return false;
                }

                // Calculate the valid time window (15 minutes after class start)
                const classEndTime = new Date(classStart);
                classEndTime.setMinutes(classStart.getMinutes() + 15);

                // Check if the current time is within the window
                return currentTime >= classStart && currentTime <= classEndTime;
            }
            ...
            function loadStudents() {
                ...
                const classStartTime = $('#class-start-time').val(); // Assuming class start time is available in the DOM

                if (!semester || !branch || !subject) return;

                const classStartDate = new Date(classStartTime); // Convert class start time to Date object
                if (!isValidTime(classStartDate)) {
                    Swal.fire('Error', 'You can only mark attendance within 15 minutes from class start time between 10 AM and 4 PM.', 'error');
            ...
            $('#submit-attendance').on('click', function () {
                ...
                const semester = $('#semester').val();
                const classStartTime = $('#class-start-time').val(); // Assuming class start time is available
