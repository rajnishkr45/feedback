  ...
  if (isset($_FILES['dp']) && $_FILES['dp']['error'] === UPLOAD_ERR_OK) {
    ...
    if (in_array(strtolower($file_ext), $allowed_ext)) {
      ...
      if (move_uploaded_file($file_tmp, '../dp/' . $new_file_name)) {
        ...
        $update_sql = "UPDATE professors SET dp = '$new_file_name' WHERE email = '$professor_email'";
...
<body>
  ...
  <section id="content">
    ...
    <main>
      ...
      <?php
      $semail = $_SESSION['prof_email'];
      $sql = "SELECT `prof_id`, `name`, `dept`, `email`, `phone`,`role`, `dp` FROM `professors` WHERE email='$semail'";
      ...
      if ($result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
          ...
          echo "<h3>Prof. " . $row['name'] . "</h3>";
          echo "<p><span>Department:</span> " . $row['dept'] . "</p>";
